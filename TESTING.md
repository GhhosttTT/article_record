...
- 页面变化是否在目标页面加载完成后记录截图元数据；同域同标签页变化是否合并进触发操作，跨域变化才作为独立页面变化步骤。
...
- 视频 SVG 帧是否把页面截图作为主画面，操作说明作为底部字幕展示，且不显示“几秒到几秒”的时间范围。
...
- 扩展预览页是否显示步骤分组，并能通过分组/步骤锚点快速定位长流程。
- SOP 文章 HTML/Markdown 是否按操作步骤导出，而不是按章节教学导出。
- 视频时间轴和 SVG 视频帧默认是否不包含 `chapter_intro` 章节开场片段。
...
- SOP 文章、Markdown、Word 兼容文档和视频时间轴导出时，有可靠打码区域的敏感步骤是否保留截图并叠加遮挡；没有可靠打码区域时是否移除原始截图 `dataUrl` 并保留隐私裁剪元数据。
...
- 同一系统同域的标签页内页面变化应写进触发操作，例如“点击 X，进入 Y 页面”，不应额外生成“跳转到 Y”的独立步骤。
...
- 导出的 SOP 文章、Markdown 和视频时间轴中，有可靠打码区域的敏感步骤应保留截图用于遮挡渲染；没有可靠打码区域的敏感步骤不应包含原始截图 `dataUrl`。
...
文章导出中应包含“操作步骤”标题，步骤编号沿用全流程顺序，不应输出“章节 1”“章节 2”等章节教学标题。

默认视频时间轴中不应包含章节开场片段；只有显式开启章节开场时才允许包含：
