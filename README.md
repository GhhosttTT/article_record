...
- 记录同一标签页内的页面跳转，等待目标页面加载完成后截图，并在文章和视频中标注来源页面和目标页面。
...
- 预览页侧栏会显示章节列表和章节内步骤锚点，长流程可以快速跳转。
...
- 导出 SOP 文章、Markdown、Word 兼容文档和视频时间轴时，含敏感信息或打码区域的步骤同样会移除原始截图 `dataUrl`，避免文件源码泄露未遮挡截图。
- 导出录制 JSON。
- 从预览页直接导出 SOP 文章 HTML、Markdown、Word 兼容 `.doc` 和视频时间轴 JSON。
- 离线工具可基于 SOP HTML 生成基础 PDF，用于归档和分享。
- SOP 文章会按连续页面和标签页上下文生成章节，长流程不再只是平铺步骤。
- 视频时间轴会为文章章节插入章节开场片段，让视频结构和文章章节保持一致。
- 可在预览页为单个步骤设置视频旁白，让视频字幕/口播和 SOP 文章说明分开调整，也可恢复使用步骤说明。
- 视频时间轴和视频帧会复用同一批步骤截图、高亮、页面跳转和打码数据。
